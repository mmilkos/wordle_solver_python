...
class UsersManagement:
    ...
    def adding_users():
        while True:
            user_id = len(users)
            is_it_going = input('Do you want to continue?(y/n): ').lower()
            if is_it_going == 'n':
                break
            ...
            users[user_id] = User(name, surname, age)

    @staticmethod
    def printing_users():
        print(users)
        for id in users:
            print(f'ID {id}: {users[id].print_info()}')


UsersManagement.adding_users()
UsersManagement.printing_users()

# metody dunder = __cos__ - są predefiniowane
# najlepie jest miec 2 pliki jeden do zdefinowanych klas a drugi do działającego kodu
...
class Rocket:
    ...
    def move_up(self):
        """ Poruszamy się do góry z prędkością speed
        """
        self.altitude += self.speed

    def __str__(self):
        return f'Rakieta jest aktualnie na wysokosći: {self.altitude} '


class RocketBoard:
    def __init__(self, amount_of_rockets):
        self.rockets = [Rocket() for _ in range(amount_of_rockets)]

        for _ in range(10):
            index_of_moving_rocket = random.randint(0, len(self.rockets) - 1)
            self.rockets[index_of_moving_rocket].move_up()

        for rocket in self.rockets:
            print(rocket)
    """wybiera konkertny element z tablicy
    """
